...
const CATEGORIES = [
  ...
  },
  {
    ...
    emoji: "🩺",
    color: "#ef4444",
    requiredQ: ["q6", "q7", "q8", "q9", "q10", "q11", "q12"],
  },
  {
    index: 2,
    id: "cat-3",
    ...
    emoji: "⏱️",
    color: "#f59e0b",
    requiredQ: ["q14", "q15", "q16"], // q13 is slider, always set
  },
  {
    index: 3,
    id: "cat-4",
...
function isCategoryDone(
  ...
  return CATEGORIES[catIndex].requiredQ.every((q) =>
...
function ChoiceGrid({
  ...
  return (
    <div
      className={`grid gap-2 ${cols === 3 ? "grid-cols-3" : "grid-cols-1 sm:grid-cols-2"}`}
    >
      {options.map((opt) => {
...
function MultiSelectGrid({
  ...
  function toggle(val: string) {
    ...
    if (without_none.includes(val)) {
      const next = without_none.filter((v) => v !== val);
      onChange(next.length === 0 ? ["none"] : next);
    } else {
      onChange([...without_none, val]);
    }
  }
        ...
        return (
          ...
          >
            ...
            <span
              ...
              style={{ color: sel ? "#f87171" : "#475569" }}
            >
              {v}
            </span>
            <span
              className="text-[9px] leading-tight text-center"
              style={{ color: sel ? "#fca5a5" : "#334155" }}
            >
              {PAIN_LABELS[v]}
            </span>
          </motion.button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <span className="text-slate-500 text-xs w-5 text-center">{min}</span>
        <input
          type="range"
          min={min}
          max={max}
        ...
        />
        <span className="text-slate-500 text-xs w-5 text-center">{max}</span>
      </div>
    </div>
  );
}
...
function ProgressBar({
  ...
  const active = firstIncomplete === -1 ? 5 : firstIncomplete;
...
export default function QuestionnairePage() {
  ...
  // Auto-scroll to next category when current is complete
  useEffect(() => {
    CATEGORIES.forEach((_, i) => {
      if (
        i < CATEGORIES.length - 1 &&
        isCategoryDone(i, answers) &&
        !scrolledCats.current.has(i)
      ) {
        scrolledCats.current.add(i);
        setTimeout(() => {
          catRefs.current[i + 1]?.scrollIntoView({
            behavior: "smooth",
            block: "start",
          });
        }, 350);
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-2 pb-40">
        ...
        <CategorySection
          cat={CATEGORIES[0]}
          done={isCategoryDone(0, answers)}
          onRef={(el) => { catRefs.current[0] = el; }}
        >
          <QBlock
            number="1"
            question="Quel est ton setup principal ?"
            answered={!!answers.q1}
          >
            <ChoiceGrid
              value={answers.q1}
          ...
          <QBlock
            number="2"
            question="Où travailles-tu le plus souvent ?"
            answered={!!answers.q2}
          >
            <ChoiceGrid
              value={answers.q2}
          ...
          <QBlock
            number="3"
            question="Ton écran est-il à hauteur des yeux ?"
            answered={!!answers.q3}
          >
            <ChoiceGrid
              value={answers.q3}
          ...
          <QBlock
            number="4"
            question="Quelle distance entre toi et ton écran ?"
            answered={!!answers.q4}
          >
            <ChoiceGrid
              value={answers.q4}
          ...
          <QBlock
            number="5"
            question="Ton clavier et ta souris sont-ils proches de toi ?"
            answered={!!answers.q5}
          >
            <ChoiceGrid
              value={answers.q5}
        ...
        <CategorySection
          cat={CATEGORIES[1]}
          done={isCategoryDone(1, answers)}
          onRef={(el) => { catRefs.current[1] = el; }}
        >
          {(
            [
              { key: "q6" as const, label: "Douleur nuque / cou", num: "6" },
              { key: "q7" as const, label: "Douleur haut du dos / épaules", num: "7" },
              { key: "q8" as const, label: "Douleur bas du dos", num: "8" },
              { key: "q9" as const, label: "Douleur poignets / avant-bras", num: "9" },
              { key: "q10" as const, label: "Douleur yeux / maux de tête", num: "10" },
            ] as const
          ).map(({ key, label, num }) => (
            <QBlock
              key={key}
          ...
          <QBlock
            number="11"
            question="Depuis combien de temps as-tu ces douleurs ?"
            answered={!!answers.q11}
          >
            <ChoiceGrid
              value={answers.q11}
          ...
          <QBlock
            number="12"
            question="Tes douleurs apparaissent quand ?"
            answered={!!answers.q12}
          >
            <ChoiceGrid
              value={answers.q12}
        ...
        {/* ── CATÉGORIE 3 — HABITUDES ── */}
        <CategorySection
          cat={CATEGORIES[2]}
          done={isCategoryDone(2, answers)}
          onRef={(el) => { catRefs.current[2] = el; }}
        >
          <QBlock
            number="13"
            question="Combien d'heures par jour es-tu assis ?"
            answered={true}
          >
            <SliderInput
              value={answers.q13}
          ...
          <QBlock
            number="14"
            question="Fais-tu des pauses pour te lever ?"
            answered={!!answers.q14}
          >
            <ChoiceGrid
              value={answers.q14}
          ...
          <QBlock
            number="15"
            question="Comment tiens-tu ton téléphone au bureau ?"
            answered={!!answers.q15}
          >
            <ChoiceGrid
              value={answers.q15}
              onChange={(v) => update("q15", v)}
              cols={3}
              options={[
                { value: "desk", label: "📱 Sur le bureau (tête droite)" },
                { value: "hand", label: "📱 En main, tête baissée" },
                { value: "headset", label: "🎧 Casque / oreillette" },
              ]}
            />
          </QBlock>
          ...
          <QBlock
            number="16"
            question="Utilises-tu un casque ou des écouteurs ?"
            answered={!!answers.q16}
          >
            <ChoiceGrid
              value={answers.q16}
              onChange={(v) => update("q16", v)}
              cols={3}
              options={[
        ...
        <CategorySection
          cat={CATEGORIES[3]}
          done={isCategoryDone(3, answers)}
          onRef={(el) => { catRefs.current[3] = el; }}
        >
          <QBlock
            number="17"
            question="Combien d'heures dors-tu par nuit ?"
            answered={true}
          >
            <SliderInput
              value={answers.q17}
          ...
          <QBlock
            number="18"
            question="Tu te réveilles comment ?"
            answered={!!answers.q18}
          >
            <ChoiceGrid
              value={answers.q18}
          ...
          <QBlock
            number="20"
            question="Ressens-tu des coups de fatigue dans la journée ?"
            answered={!!answers.q20}
          >
            <ChoiceGrid
              value={answers.q20}
              onChange={(v) => update("q20", v)}
              options={[
                { value: "never", label: "✅ Jamais" },
        ...
        <CategorySection
          cat={CATEGORIES[4]}
          done={isCategoryDone(4, answers)}
          onRef={(el) => { catRefs.current[4] = el; }}
        >
          <QBlock
            number="21"
            question="As-tu déjà eu un diagnostic médical lié à ta posture ? (plusieurs réponses possibles)"
            answered={answers.q21.length > 0}
          >
            <MultiSelectGrid
              value={answers.q21}
              ...
              options={[
                ...
                { value: "hernie", label: "🦴 Hernie discale" },
                { value: "tendinite", label: "💪 Tendinite" },
                { value: "cervicalgie", label: "🔴 Cervicalgie" },
          ...
          <QBlock
            number="22"
            question="Fais-tu du sport ?"
            answered={!!answers.q22}
          >
            <ChoiceGrid
              value={answers.q22}
              onChange={(v) => update("q22", v)}
              options={[
                { value: "never", label: "❌ Jamais" },
          ...
          <QBlock
            number="23"
            question="Fais-tu des exercices d'étirement ?"
            answered={!!answers.q23}
          >
            <ChoiceGrid
              value={answers.q23}
          ...
          <QBlock
            number="24"
            question="Comment décris-tu ta posture spontanément ?"
            answered={!!answers.q24}
          >
            <ChoiceGrid
              value={answers.q24}
              onChange={(v) => update("q24", v)}
              options={[
        ...
        {/* ── CATÉGORIE 6 — RESSENTI ── */}
        <CategorySection
          cat={CATEGORIES[5]}
          done={isCategoryDone(5, answers)}
          onRef={(el) => { catRefs.current[5] = el; }}
        >
          <QBlock
            number="25"
            question="Comment tu te sens au travail en ce moment ?"
            answered={answers.q25 !== null}
          >
            <WellbeingScale
              value={answers.q25}
